  ...
  try {
    ...
    if (payload.activate_all === true) {
      return jsonResponse(400, {
        ...
        message: "Full 47,979 activation is blocked. Use only the approved 250-agent company.",
      });
    }
    ...
    if (batchSize > 250) {
      return jsonResponse(400, {
        ok: false,
        error: "COMPANY_ACTIVATION_BLOCKED",
        message: "batch_size exceeds 250. Activation beyond 250 is blocked.",
      });
    }
    ...
    if (requestedIds.length !== agentIds.length) {
      const blocked = await supabaseRpc("runtime_company_activate_agents", {
      ...
      return jsonResponse(409, {
        ...
        reason: blocked.reason || "Only the approved 250-agent company may be activated.",
        unknown_agent_ids: blocked.unknown_agent_ids || null,
        audit_event: blocked.audit_event || null,
      });
    }

    const result = await supabaseRpc("runtime_company_activate_agents", {
    ...
    return jsonResponse(result.blocked ? 409 : 200, {
      ...
      operation: "activate-runtime-company",
    ...
    return jsonResponse(500, {
      ok: false,
      error: "Runtime company activation failed.",
    });
  }
};
