...
exports.handler = async function handler(event) {
  ...
  try {
    ...
    if (payload.execution_enabled === true) {
    ...
    if (payload.activate_all === true) {
      return jsonResponse(400, {
        ...
        message: "Full 47,979 deactivation is blocked. Only the approved 10-agent squad may be managed.",
      });
    }
    ...
    if (batchSize > 10) {
      return jsonResponse(400, {
        ok: false,
        error: "SQUAD_DEACTIVATION_BLOCKED",
        message: "batch_size exceeds 10. Mass deactivation beyond 10 is blocked.",
      });
    }
    ...
    const requestedIds = agentIds.filter((id) => ALLOWED_SQUAD_AGENTS.includes(id));
    if (requestedIds.length !== agentIds.length) {
      const blocked = await supabaseRpc("runtime_squad_deactivate_agents", {
      ...
      return jsonResponse(409, {
        ...
        reason: blocked.reason || "Only the approved 10-agent squad may be managed.",
        unknown_agent_ids: blocked.unknown_agent_ids || null,
        audit_event: blocked.audit_event || null,
      });
    }

    const result = await supabaseRpc("runtime_squad_deactivate_agents", {
    ...
    return jsonResponse(result.blocked ? 409 : 200, {
      ...
      operation: "deactivate-runtime-squad",
    ...
    return jsonResponse(500, {
      ok: false,
      error: "Runtime squad deactivation failed.",
    });
  }
};
