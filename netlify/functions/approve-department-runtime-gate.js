...
exports.handler = async function handler(event) {
  ...
  try {
    ...
    const activationCap = Number(payload.activation_cap);
    ...
    if (!Number.isInteger(activationCap) || activationCap < 1 || activationCap > gate.MAX_DEPARTMENT_ACTIVATION_CAP) {
      return base.jsonResponse(400, {
        ok: false,
        error: "activation_cap must be between 1 and 250.",
      });
    }
    ...
    const result = await gate.supabaseRpc("approve_department_runtime_gate", {
      p_department_id: departmentId,
      p_activation_cap: activationCap,
    ...
    if (!result.ok) {
      return base.jsonResponse(result.blocked ? 409 : 400, {
        ok: false,
        operation: "approve-department-runtime-gate",
        ...result,
      });
    }
    ...
    return base.jsonResponse(500, {
      ok: false,
      error: "Department gate approval failed.",
    });
  }
};
