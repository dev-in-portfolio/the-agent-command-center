  ...
  try {
    ...
    if (
      ...
      return jsonResponse(400, {
        ...
        message: "Mass activation is blocked. Only one supervised test agent may be activated at a time.",
      });
    }
    ...
    if (agentId !== "mvp53_supervised_test_agent_001") {
      const blocked = await supabaseRpc("runtime_agent_activation_activate", {
        p_agent_id: agentId,
      ...
      return jsonResponse(409, {
        ok: false,
        blocked: true,
        error: blocked.error || "ACTIVATION_BLOCKED",
        reason: blocked.reason || "Only the supervised test agent may be activated.",
        event: blocked.event || null,
      });
    }

    const result = await supabaseRpc("runtime_agent_activation_activate", {
      p_agent_id: agentId,
      p_actor: actor || "operator",
      p_reason: reason,
    });
