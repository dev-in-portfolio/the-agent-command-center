...
exports.handler = async function handler(event) {
  ...
  try {
    ...
    const laneKey = text(payload.lane_key || payload.scope_key, 80);
    const actor = text(payload.actor_name, 120);
    const reason = text(payload.reason, 2000);

    const result = await supabaseRpc("runtime_division_activate_agents", {
      p_agent_ids: null,
      p_actor: actor || "operator",
      p_reason: reason,
      p_batch_size: Number(payload.batch_size || 10),
      p_chunk_size: Number(payload.chunk_size || 100),
      p_activate_all: false,
      p_scope: "lane",
      p_scope_key: laneKey || null,
    });

    return jsonResponse(result.blocked ? 409 : 200, {
      ok: Boolean(result.ok),
      operation: "activate-division-lane",
    ...
    return jsonResponse(500, { ok: false, error: "Runtime division lane activation failed." });
  }
};
