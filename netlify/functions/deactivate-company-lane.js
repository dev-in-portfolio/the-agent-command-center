...
exports.handler = async function handler(event) {
  ...
  try {
    ...
    if (!ALLOWED_COMPANY_LANES.some((lane) => lane.lane_key === laneKey)) {
      return jsonResponse(400, {
        ok: false,
    ...
    const result = await supabaseRpc("runtime_company_deactivate_lane", {
      p_lane_key: laneKey,
    ...
    return jsonResponse(result.blocked ? 409 : 200, {
      ...
      operation: "deactivate-company-lane",
    ...
    return jsonResponse(500, {
      ok: false,
      error: "Company lane deactivation failed.",
    });
  }
};
