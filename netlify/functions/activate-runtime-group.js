  ...
  try {
    ...
    if (payload.activate_all === true) {
      return jsonResponse(400, {
        ...
        message: "Full 47,979 activation is blocked. Use only the approved 500-agent group.",
      });
    }
    ...
    if (batchSize > 500) {
      return jsonResponse(400, {
        ok: false,
        error: "GROUP_ACTIVATION_BLOCKED",
        message: "batch_size exceeds 500. Activation beyond 500 is blocked.",
      });
    }
    ...
    if (requestedIds.length !== agentIds.length) {
      const blocked = await supabaseRpc("runtime_group_activate_agents", {
      ...
      return jsonResponse(409, {
        ...
        reason: blocked.reason || "Only the approved 500-agent group may be activated.",
        unknown_agent_ids: blocked.unknown_agent_ids || null,
        audit_event: blocked.audit_event || null,
      });
    }

    const result = await supabaseRpc("runtime_group_activate_agents", {
    ...
    return jsonResponse(result.blocked ? 409 : 200, {
      ...
      operation: "activate-runtime-group",
    ...
    return jsonResponse(500, {
      ok: false,
      error: "Runtime group activation failed.",
    });
  }
};
