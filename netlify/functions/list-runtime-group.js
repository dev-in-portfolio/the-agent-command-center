...
exports.handler = async function handler(event) {
  ...
  try {
    const [configRows, laneRows, agentRows, heartbeatRows, noteRows, activationRows, auditRows] = await Promise.all([
      ...
      supabaseGet("runtime_group_lanes?select=*&order=lane_order.asc"),
      supabaseGet("runtime_group_agents?select=*&order=lane_key.asc,lane_position.asc"),
      supabaseGet("group_heartbeat_events?select=*&order=created_at.desc&limit=500"),
      supabaseGet("group_readiness_notes?select=*&order=created_at.desc&limit=500"),
      supabaseGet("runtime_group_activation_events?select=*&order=created_at.desc&limit=500"),
      supabaseGet("runtime_group_audit_events?select=*&order=created_at.desc&limit=500"),
    ]);
    ...
    const groupRollup = config.group_health_rollup || {
      group_health: activeAgents === 0 ? "inactive" : activeAgents >= 500 && activeLaneCount >= 50 ? "healthy" : "partial",
      active_agents: activeAgents,
      inactive_agents: Math.max(500 - activeAgents, 0),
    ...
    return jsonResponse(200, {
      ...
      backend_status: {
        ...
        runtime_group_size: Number(config.runtime_group_size || 500),
        live_runtime_agents_enabled: activeAgents,
        max_activation_batch_size: Number(config.max_activation_batch_size || 500),
        ...
        group_health_rollup: groupRollup,
      },
      config,
    ...
    return jsonResponse(500, {
      ok: false,
      error: "Runtime group list failed.",
    });
  }
};
