  ...
  try {
    ...
    if (payload.activate_all === true) {
      return jsonResponse(400, {
        ...
        message: "Full 47,979 deactivation is blocked. Use only the approved 100-agent battalion.",
      });
    }
    ...
    if (batchSize > 100) {
      return jsonResponse(400, {
        ok: false,
        error: "BATTALION_DEACTIVATION_BLOCKED",
        message: "batch_size exceeds 100. Deactivation beyond 100 is blocked.",
      });
    }
    ...
    if (requestedIds.length !== agentIds.length) {
      const blocked = await supabaseRpc("runtime_battalion_deactivate_agents", {
      ...
      return jsonResponse(409, {
        ...
        reason: blocked.reason || "Only the approved 100-agent battalion may be managed.",
        unknown_agent_ids: blocked.unknown_agent_ids || null,
        audit_event: blocked.audit_event || null,
      });
    }

    const result = await supabaseRpc("runtime_battalion_deactivate_agents", {
    ...
    return jsonResponse(result.blocked ? 409 : 200, {
      ...
      operation: "deactivate-runtime-battalion",
    ...
    return jsonResponse(500, {
      ok: false,
      error: "Runtime battalion deactivation failed.",
    });
  }
};
