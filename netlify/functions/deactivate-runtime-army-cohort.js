...
exports.handler = async function handler(event) {
  ...
  try {
    ...
    const cohortId = army.text(payload.cohort_id, 80);
    const actor = army.text(payload.actor, 120);
    const reason = army.text(payload.reason, 4000);

    if (!cohortId) {
      return base.jsonResponse(400, { ok: false, error: "cohort_id is required." }, "mvp62-20000-agent-department-gated-runtime-army");
    }

    const result = await army.supabaseRpc("runtime_army_deactivate_cohort", {
    ...
    if (!result.ok) {
      return base.jsonResponse(result.blocked ? 409 : 400, { ok: false, operation: "deactivate-runtime-army-cohort", ...result }, "mvp62-20000-agent-department-gated-runtime-army");
    }

    return base.jsonResponse(200, {
      ok: true,
      operation: "deactivate-runtime-army-cohort",
      blocked: false,
    ...
    return base.jsonResponse(500, { ok: false, error: "Runtime army cohort deactivation failed." }, "mvp62-20000-agent-department-gated-runtime-army");
  }
};
