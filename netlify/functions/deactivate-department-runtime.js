...
exports.handler = async function handler(event) {
  ...
  try {
    ...
    const result = await gate.supabaseRpc("deactivate_department_runtime", {
      p_department_id: departmentId,
      p_actor: actor || "operator",
      p_reason: reason || "operator deactivation",
    });

    if (!result.ok) {
      return base.jsonResponse(result.blocked ? 409 : 400, {
        ok: false,
        operation: "deactivate-department-runtime",
        ...result,
      });
    }
    ...
    return base.jsonResponse(500, {
      ok: false,
      error: "Department runtime deactivation failed.",
    });
  }
};
