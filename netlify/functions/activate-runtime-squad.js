  ...
  try {
    ...
    if (payload.activate_all === true) {
      return jsonResponse(400, {
        ...
        message: "Full 47,979 activation is blocked. Only the approved 10-agent squad may be managed.",
      });
    }
    ...
    if (batchSize > 10) {
      return jsonResponse(400, {
        ok: false,
        error: "SQUAD_ACTIVATION_BLOCKED",
        message: "batch_size exceeds 10. Mass activation beyond 10 is blocked.",
      });
    }
    ...
    if (requestedIds.length !== agentIds.length) {
      const blocked = await supabaseRpc("runtime_squad_activate_agents", {
      ...
      return jsonResponse(409, {
        ...
        reason: blocked.reason || "Only the approved 10-agent squad may be activated.",
        unknown_agent_ids: blocked.unknown_agent_ids || null,
        audit_event: blocked.audit_event || null,
      });
    }

    const result = await supabaseRpc("runtime_squad_activate_agents", {
    ...
    return jsonResponse(result.blocked ? 409 : 200, {
      ...
      operation: "activate-runtime-squad",
    ...
    return jsonResponse(500, {
      ok: false,
      error: "Runtime squad activation failed.",
    });
  }
};
