  ...
  try {
    ...
    if (payload.command_execution_enabled === true || payload.deploy_execution_enabled === true || payload.rollback_execution_enabled === true || payload.alert_sending_enabled === true) {
      return jsonResponse(400, {
        ok: false,
        error: "COMPANY_EXECUTION_DISABLED",
      });
    }
    ...
    if (payload.activate_all === true) {
      return jsonResponse(400, {
        ...
        message: "Full 47,979 deactivation is blocked. Use only the approved 250-agent company.",
      });
    }
    ...
    if (batchSize > 250) {
      return jsonResponse(400, {
        ok: false,
        error: "COMPANY_DEACTIVATION_BLOCKED",
        message: "batch_size exceeds 250. Deactivation beyond 250 is blocked.",
      });
    }
    ...
    const requestedIds = agentIds.filter((id) => ALLOWED_COMPANY_AGENTS.includes(id));
    if (requestedIds.length !== agentIds.length) {
      const blocked = await supabaseRpc("runtime_company_deactivate_agents", {
      ...
      return jsonResponse(409, {
        ...
        reason: blocked.reason || "Only the approved 250-agent company may be managed.",
        unknown_agent_ids: blocked.unknown_agent_ids || null,
        audit_event: blocked.audit_event || null,
      });
    }

    const result = await supabaseRpc("runtime_company_deactivate_agents", {
    ...
    return jsonResponse(result.blocked ? 409 : 200, {
      ...
      operation: "deactivate-runtime-company",
    ...
    return jsonResponse(500, {
      ok: false,
      error: "Runtime company deactivation failed.",
    });
  }
};
