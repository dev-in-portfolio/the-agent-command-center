...
exports.handler = async function handler(event) {
  ...
  try {
    ...
    const result = await supabaseRpc("runtime_division_create_readiness_note", {
      ...
      p_note_title: text(payload.note_title, 200) || null,
      p_note_body: text(payload.note_body, 4000) || null,
      p_readiness_level: text(payload.readiness_level || "green", 40) || "green",
    });

    return jsonResponse(result.blocked ? 409 : 200, {
      ok: Boolean(result.ok),
      operation: "create-division-readiness-note",
    ...
    return jsonResponse(500, { ok: false, error: "Runtime division readiness note failed." });
  }
};
