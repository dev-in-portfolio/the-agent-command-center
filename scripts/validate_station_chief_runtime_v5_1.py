...
V5_1_MODULE = REPO_ROOT / "10_runtime" / "station_chief_first_supervised_local_execution_kernel_candidate.py"
...
REPORT = REPO_ROOT / "09_exports" / "station_chief_runtime_v5_1_report.md"
ADAPTERS = REPO_ROOT / "10_runtime" / "station_chief_adapters.py"
RELEASE_LOCK = REPO_ROOT / "10_runtime" / "station_chief_release_lock.py"

EXPECTED_V5_1_TOKEN = "YES_I_APPROVE_FIRST_SUPERVISED_LOCAL_EXECUTION_KERNEL_CANDIDATE"
HUMAN_OPERATOR = "Devin O’Rourke"
SYNTHETIC_TASK_LABEL = "sandbox status note"
...
def ensure_required_files() -> None:
    for path in [
        ...
        V5_1_MODULE,
        V5_0_VALIDATOR,
        REPO_ROOT / "09_exports" / "station_chief_runtime_v5_0_report.md",
        REPO_ROOT / "09_exports" / "station_chief_v5_0_first_live_queue_execution_candidate_review_preflight_audit.md",
        REPO_ROOT / "09_exports" / "station_chief_v5_1_first_supervised_local_execution_kernel_candidate_preflight_audit.md",
        README,
        SKELETON,
        REPORT,
        ADAPTERS,
...
def ensure_versions() -> None:
    ...
    ensure(runtime["STATION_CHIEF_RUNTIME_VERSION"] == "5.1.0", "runtime version mismatch")
    ensure(runtime["generate_run_id"]("check please").startswith("station-chief-v5-1-"), "run id prefix mismatch")
    runtime_result = runtime["run_station_chief"]("check please")
    ensure(runtime_result["runtime_status"] == "first_supervised_local_execution_kernel_candidate", "runtime status mismatch")
    ensure(adapters["ADAPTER_MODULE_VERSION"] == "5.1.0", "adapter module version mismatch")
    noop = adapters["SUPPORTED_ADAPTERS"]["noop"]
    ensure(noop["supports_first_supervised_local_execution_kernel_candidate"] is True, "adapter v5.1 support mismatch")
    ensure(noop["first_supervised_local_execution_kernel_candidate_requires_specific_token"] is True, "adapter v5.1 token metadata mismatch")
    ensure(noop["one_local_supervised_output_record_allowed_with_v5_1_token"] is True, "adapter v5.1 record allowance mismatch")
    ensure(noop["deterministic_local_output_write_allowed"] is True, "adapter deterministic local output allowance mismatch")
    ...
    ensure(noop["arbitrary_task_execution_allowed"] is False, "adapter arbitrary task execution denial mismatch")
    ensure(noop["user_task_execution_allowed"] is False, "adapter user task execution denial mismatch")
    ...
    ensure(release_lock["STABLE_RUNTIME_VERSION"] == "5.1.0", "release lock version mismatch")


def ensure_module_exports() -> None:
    module = load_script(V5_1_MODULE)
    for name in [
        "FIRST_SUPERVISED_LOCAL_EXECUTION_KERNEL_CANDIDATE_MODULE_VERSION",
        "FIRST_SUPERVISED_LOCAL_EXECUTION_KERNEL_CANDIDATE_STATUS",
        "FIRST_SUPERVISED_LOCAL_EXECUTION_KERNEL_CANDIDATE_PHASE",
        "FIRST_SUPERVISED_LOCAL_EXECUTION_KERNEL_CANDIDATE_APPROVAL_TOKEN",
        "DEFAULT_SYNTHETIC_TASK_LABEL",
        "DEFAULT_SUPERVISED_LOCAL_OUTPUT_RECORD_NAME",
    ]:
        ensure(name in module, f"missing v5.1 module constant: {name}")
    for name in [
        "canonical_json",
        "sha256_digest",
        "normalize_label",
        "safe_output_record_name",
        "generate_supervised_local_execution_kernel_id",
        "create_first_supervised_local_execution_kernel_candidate_schema",
        "create_supervised_execution_kernel_approval_gate",
        "create_synthetic_task_contract",
        "create_sandbox_output_scope_contract",
        "create_non_external_execution_boundary",
        "create_execution_permission_denial_record",
        "create_supervised_local_execution_plan_record",
        "build_supervised_local_execution_output_payload",
        "write_first_supervised_local_execution_kernel_output_record",
        "create_blocked_supervised_local_execution_write_record",
        "create_supervised_local_execution_result_record",
        "create_supervised_local_execution_audit_record",
        "create_supervised_local_execution_readiness_summary",
        "create_controlled_repeatable_local_execution_candidate_bridge",
        "create_first_supervised_local_execution_kernel_candidate_bundle",
    ]:
        ensure(name in module and callable(module[name]), f"missing v5.1 module export: {name}")


def ensure_no_forbidden_patterns() -> None:
    text = V5_1_MODULE.read_text(encoding="utf-8")
    for pattern in FORBIDDEN_REGEXES:
        ensure(re.search(pattern, text) is None, f"forbidden pattern found in v5.1 module: {pattern}")
...
def ensure_cli_flags() -> None:
    ...
    for flag in [
        "--first-supervised-local-execution-kernel-candidate-schema",
        "--first-supervised-local-execution-kernel-candidate",
        "--write-first-supervised-local-execution-kernel-candidate",
        "--v5-synthetic-task-label",
        "--v5-supervised-output-record-name",
        "--v5-supervised-execution-confirm-token",
        "--v5-supervised-execution-human-operator",
    ]:
        ensure(flag in stdout, f"missing CLI flag: {flag}")


def ensure_schema_and_gates() -> None:
    schema = run_json(RUNTIME, ["--first-supervised-local-execution-kernel-candidate-schema"])
    ensure(schema["schema_version"] == "5.1.0", "schema version mismatch")
    ensure(schema["status"] == "FIRST_SUPERVISED_LOCAL_EXECUTION_KERNEL_CANDIDATE_LOCAL_OUTPUT_ONLY", "schema status mismatch")
    ensure(schema["execution_type"] == "first_supervised_local_execution_kernel_candidate", "schema execution type mismatch")
    ensure(schema["required_token"] == EXPECTED_V5_1_TOKEN, "schema token mismatch")
    for key in [
    ...
    ensure(schema["local_supervised_output_record_written"] is False, "schema write flag must be false")
    ensure(schema["supervised_local_execution_performed"] is False, "schema execution flag must be false")
    ...
    ensure(schema["arbitrary_task_execution_performed"] is False, "schema arbitrary task flag must be false")
    ensure(schema["user_task_execution_performed"] is False, "schema user task flag must be false")
    ...
    ensure(schema["external_tool_invocation_performed"] is False, "schema external tool flag must be false")
    ensure(schema["api_call_performed"] is False, "schema api flag must be false")
    ensure(schema["network_access_performed"] is False, "schema network flag must be false")
    ensure(schema["deployment_performed"] is False, "schema deployment flag must be false")
    ensure(schema["production_execution_performed"] is False, "schema production flag must be false")
    ...
    ensure(no_token["supervised_execution_kernel_approval_gate"]["gate_status"] == "BLOCKED_PENDING_V5_1_SUPERVISED_LOCAL_EXECUTION_APPROVAL", "no-token path should block")
    ensure(no_token["supervised_local_execution_write_record"]["write_status"] == "BLOCKED", "no-token write record should block")
    ensure(no_token["local_supervised_output_record_written"] is False, "no-token bundle should not write")
    ...
    ensure(bad_token["supervised_execution_kernel_approval_gate"]["gate_status"] == "BLOCKED_PENDING_V5_1_SUPERVISED_LOCAL_EXECUTION_APPROVAL", "bad-token path should block")
    ensure(bad_token["supervised_local_execution_write_record"]["write_status"] == "BLOCKED", "bad-token write record should block")
    ...
    gate = valid_no_write["supervised_execution_kernel_approval_gate"]
    ensure(gate["gate_status"] == "APPROVED_FOR_ONE_SUPERVISED_LOCAL_OUTPUT_RECORD", "valid token should approve one record")
    ensure(gate["local_supervised_execution_records_authorized"] is True, "valid token should authorize local records")
    ensure(gate["local_supervised_output_record_write_authorized"] is False, "preview request absent should block write")
    reference = valid_no_write["synthetic_task_contract"]
    scope = valid_no_write["sandbox_output_scope_contract"]
    boundary = valid_no_write["non_external_execution_boundary"]
    denial = valid_no_write["execution_permission_denial_record"]
    candidate = valid_no_write["supervised_local_execution_plan_record"]
    result_record = valid_no_write["supervised_local_execution_result_record"]
    audit = valid_no_write["supervised_local_execution_audit_record"]
    summary = valid_no_write["supervised_local_execution_readiness_summary"]
    bridge = valid_no_write["controlled_repeatable_local_execution_candidate_bridge"]
    ensure(reference["contract_status"] == "CREATED", "synthetic task contract should be created")
    ensure(scope["scope_status"] == "PASS", "scope should pass")
    ensure(boundary["boundary_status"] == "PASS", "boundary should pass")
    ensure(candidate["plan_status"] == "LOCAL_SUPERVISED_EXECUTION_PLAN_CREATED", "plan should be created")
    ensure(result_record["result_status"] == "BLOCKED", "result should be blocked without write request")
    ensure(audit["audit_status"] == "PASS", "audit should pass")
    ensure(summary["readiness_status"] == "READY_FOR_CONTROLLED_REPEATABLE_LOCAL_EXECUTION_CANDIDATE_REVIEW_ONLY", "summary should be ready")
    ensure(bridge["bridge_status"] == "READY_FOR_CONTROLLED_REPEATABLE_LOCAL_EXECUTION_CANDIDATE_REVIEW_ONLY", "bridge should be ready")
    ensure(valid_no_write["supervised_local_execution_write_record"]["write_status"] == "BLOCKED", "no-write path should block write")
    ensure(valid_no_write["local_supervised_output_record_written"] is False, "no-write bundle should not write")
    ...
    ensure(valid_no_write["arbitrary_task_execution_performed"] is False, "no-write bundle arbitrary task should be false")
    ensure(valid_no_write["user_task_execution_performed"] is False, "no-write bundle user task should be false")
    ...
    ensure(valid_no_write["external_tool_invocation_performed"] is False, "no-write bundle external tool should be false")
    ensure(valid_no_write["api_call_performed"] is False, "no-write bundle api call should be false")
    ensure(valid_no_write["network_access_performed"] is False, "no-write bundle network access should be false")
    ensure(valid_no_write["deployment_performed"] is False, "no-write bundle deployment should be false")
    ensure(valid_no_write["production_execution_performed"] is False, "no-write bundle production execution should be false")
    ensure(valid_no_write["full_workforce_activation_performed"] is False, "no-write bundle workforce activation should be false")
    ensure(valid_no_write["supervised_local_execution_output_payload"] is None, "no-write path should not expose a payload")

    with tempfile.TemporaryDirectory(prefix="station_chief_v5_1_", dir="/tmp") as review_dir:
        ...
        write_record = write_result["supervised_local_execution_write_record"]
        ensure(write_record["write_status"] == "SUPERVISED_LOCAL_OUTPUT_RECORD_WRITTEN", "write path should write one output record")
        ensure(write_record["local_supervised_output_record_written"] is True, "write record should mark local write")
        ensure(write_record["supervised_local_execution_performed"] is True, "write record should mark supervised execution")
        ensure(write_record["files_written_count"] == 1, "write record should report exactly one file")
        ensure(write_result["local_supervised_output_record_written"] is True, "bundle should report local write")
        ensure(write_result["supervised_local_execution_performed"] is True, "bundle should report supervised execution")
        ensure(write_result["supervised_local_execution_result_record"]["result_status"] == "SUPERVISED_LOCAL_OUTPUT_RECORD_WRITTEN", "result record should reflect write")
        written_files = list(Path(review_dir).iterdir())
        ensure(len(written_files) == 1, f"expected exactly one output record, found {len(written_files)}")
        review_path = written_files[0]
        ensure(review_path.name == DEFAULT_OUTPUT_RECORD_NAME, "output record filename mismatch")
        ensure(review_path.resolve().is_relative_to(Path(review_dir).resolve()), "output record escaped output directory")
        review_payload = json.loads(review_path.read_text(encoding="utf-8"))
        ensure(review_payload["runtime_version"] == "5.1.0", "review payload runtime version mismatch")
        ensure(review_payload["execution_type"] == "first_supervised_local_execution_kernel_candidate", "review payload type mismatch")
        ensure(review_payload["timestamp_mode"] == "deterministic_no_wall_clock_timestamp", "review payload timestamp mode mismatch")
        ensure(review_payload["output_message"] == "Station Chief supervised local execution kernel wrote this deterministic sandbox output record.", "review payload message mismatch")
        ...
        ensure(review_payload["arbitrary_task_execution_performed"] is False, "review payload arbitrary task should be false")
        ensure(review_payload["user_task_execution_performed"] is False, "review payload user task should be false")
        ...
        ensure(review_payload["supervised_local_execution_performed"] is True, "review payload supervised local execution should be true")
        ensure(review_payload["external_tool_invocation_performed"] is False, "review payload external tool should be false")
        ensure(review_payload["api_call_performed"] is False, "review payload api call should be false")
        ensure(review_payload["network_access_performed"] is False, "review payload network access should be false")
        ensure(review_payload["deployment_performed"] is False, "review payload deployment should be false")
        ensure(review_payload["production_execution_performed"] is False, "review payload production execution should be false")
        ...
        ensure(write_result["supervised_local_execution_write_record"]["record_path"] == str(review_path), "write record path mismatch")
...
def ensure_smoke_tests() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="station_chief_v5_3_hidden_", dir="/tmp") as hidden_dir_name:
        ...
        try:
            ...
            for validator in [V5_0_VALIDATOR, V4_9_VALIDATOR, V4_8_VALIDATOR, V4_7_VALIDATOR]:
...
def main() -> None:
    ...
    ensure_no_v62_files()
    print("STATION_CHIEF_RUNTIME_V5_1_VALIDATION_PASS")
