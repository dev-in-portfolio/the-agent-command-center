...
def main():
    ...
    # Final secret/write-scope scan
    stdout, stderr, code = run("python3 - <<'PY'\nfrom pathlib import Path\nscan_roots = [Path('14_backend/product_runtime/providers/supabase'), Path('netlify/functions'), Path('13_web_dashboard/dist'), Path('09_exports/mvp_product_track')]\nfor root in scan_roots:\n if not root.exists(): continue\n for path in root.rglob('*'):\n  if not path.is_file(): continue\n  text = path.read_text(encoding='utf-8', errors='replace')\n  lower = text.lower()\n  if 'sb_secret_' in text: raise SystemExit(f'SECRET_KEY_LEAK: {path}')\n  if 'postgresql://postgres:' in text: raise SystemExit(f'POSTGRES_CONNECTION_STRING_LEAK: {path}')\n  if 'SUPABASE_SERVICE_ROLE_KEY=sb_' in text: raise SystemExit(f'SERVICE_ROLE_VALUE_LEAK: {path}')\n  if path.name == 'supabase_write_client.js':\n   for item in ['SUPABASE_SERVICE_ROLE_KEY', 'service_role', 'method: \"PATCH\"', 'method: \"DELETE\"', '/rpc/']:\n    if item.lower() in lower: raise SystemExit(f'FORBIDDEN_WRITE_CLIENT_PATTERN {item}: {path}')\nprint('MVP8_SECRET_SCAN_PASS')\nPY")
    if code != 0:
        fail(f"Secret scan failed: {stdout}")

    print("MVP8_CONTROLLED_AUTHENTICATED_REQUEST_CREATE_E2E_VALIDATION_PASS")
