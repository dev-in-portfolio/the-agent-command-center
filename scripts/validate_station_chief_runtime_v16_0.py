...
def main():
    ...
    # Versions
    check_file_content(runtime_dir / "station_chief_runtime.py", r'STATION_CHIEF_RUNTIME_VERSION\s*=\s*"16\.0\.0"', "Runtime version not 16.0.0")
    check_file_content(runtime_dir / "station_chief_release_lock.py", r'STABLE_RUNTIME_VERSION\s*=\s*"16\.0\.0"', "Release lock not 16.0.0")
    check_file_content(runtime_dir / "station_chief_adapters.py", r'ADAPTER_MODULE_VERSION\s*=\s*"16\.0\.0"', "Adapter version not 16.0.0")

    # Future files
    ensure(not list(root_dir.rglob("*v16_1*")), "v16.1 files exist")
    ensure(not list(root_dir.rglob("*v16.1*")), "v16.1 files exist")
    ensure(not list(root_dir.rglob("*v17*")), "v17 files exist")
