...
V5_7_MODULE = REPO_ROOT / "10_runtime" / "station_chief_sandbox_worker_dry_run_assignment_candidate.py"
...
REPORT = REPO_ROOT / "09_exports" / "station_chief_runtime_v5_7_report.md"
AUDIT = REPO_ROOT / "09_exports" / "station_chief_v5_7_sandbox_worker_dry_run_assignment_candidate_preflight_audit.md"
ADAPTERS = REPO_ROOT / "10_runtime" / "station_chief_adapters.py"
RELEASE_LOCK = REPO_ROOT / "10_runtime" / "station_chief_release_lock.py"

EXPECTED_TOKEN = "YES_I_APPROVE_SANDBOX_WORKER_DRY_RUN_ASSIGNMENT_CANDIDATE"
...
def ensure_required_files() -> None:
    for path in [RUNTIME, V5_7_MODULE, V5_6_VALIDATOR, V5_4_VALIDATOR, V5_3_VALIDATOR, V5_2_VALIDATOR, V5_1_VALIDATOR, V5_0_VALIDATOR, README, SKELETON, REPORT, AUDIT, ADAPTERS, RELEASE_LOCK]:
        ensure(path.exists(), f"missing required file: {path.relative_to(REPO_ROOT)}")


def ensure_versions() -> None:
    runtime = load_script(RUNTIME)
    module = load_script(V5_7_MODULE)
    adapters = load_script(ADAPTERS)
    release_lock = load_script(RELEASE_LOCK)
    ensure(runtime["STATION_CHIEF_RUNTIME_VERSION"] == "5.7.0", "runtime version mismatch")
    ensure(runtime["generate_run_id"]("check please").startswith("station-chief-v5-7-"), "run id prefix mismatch")
    ensure(runtime["run_station_chief"]("check please")["runtime_status"] == "sandbox_worker_dry_run_assignment_candidate", "runtime status mismatch")
    ensure(adapters["ADAPTER_MODULE_VERSION"] == "5.7.0", "adapter module version mismatch")
    noop = adapters["SUPPORTED_ADAPTERS"]["noop"]
    ensure(noop["supports_sandbox_worker_dry_run_assignment_candidate"] is True, "adapter dry_run_assignment_candidate support mismatch")
    ensure(noop["sandbox_worker_dry_run_assignment_candidate_requires_specific_token"] is True, "adapter token mismatch")
    ensure(noop["one_local_sandbox_worker_dry_run_assignment_candidate_allowed_with_v5_7_token"] is True, "adapter packet allowance mismatch")
    ensure(noop["deterministic_local_dry_run_assignment_packet_write_allowed"] is True, "adapter deterministic write mismatch")
    ...
    ensure(release_lock["STABLE_RUNTIME_VERSION"] == "5.7.0", "release lock version mismatch")
    ensure(module["SANDBOX_WORKER_DRY_RUN_ASSIGNMENT_CANDIDATE_MODULE_VERSION"] == "5.7.0", "module constant mismatch")
    ensure(module["SANDBOX_WORKER_DRY_RUN_ASSIGNMENT_CANDIDATE_STATUS"] == "SANDBOX_WORKER_DRY_RUN_ASSIGNMENT_CANDIDATE_LOCAL_PACKET_ONLY", "module status mismatch")
    ensure(module["SANDBOX_WORKER_DRY_RUN_ASSIGNMENT_CANDIDATE_PHASE"] == "Sandbox Worker Dry-Run Assignment Candidate", "module phase mismatch")
    ensure(module["SANDBOX_WORKER_DRY_RUN_ASSIGNMENT_CANDIDATE_APPROVAL_TOKEN"] == EXPECTED_TOKEN, "module token mismatch")
    ...
    ensure(module["DEFAULT_SANDBOX_DRY_RUN_ASSIGNMENT_PACKET_NAME"] == DEFAULT_PACKET_NAME, "module packet default mismatch")
...
def ensure_module_exports() -> None:
    ...
    for name in [
        ...
        "sha256_digest",
        "normalize_label",
        "safe_dry_run_assignment_packet_name",
        "generate_sandbox_worker_dry_run_assignment_candidate_id",
        "create_sandbox_worker_dry_run_assignment_candidate_schema",
        "create_sandbox_worker_dry_run_assignment_approval_gate",
        "create_v5_3_handoff_packet_reference_contract",
        "create_v5_4_acknowledgement_packet_reference_contract",
        "create_v5_6_ready_state_packet_reference_contract",
        "create_sandbox_worker_dry_run_assignment_reference_contract",
        "create_dry_run_assignment_scope_contract",
        "create_non_execution_dry_run_assignment_boundary",
        "create_dry_run_assignment_permission_denial_record",
        "create_dry_run_assignment_plan_record",
        "build_dry_run_assignment_packet_payload",
        "write_sandbox_worker_dry_run_assignment_packet",
        "create_blocked_dry_run_assignment_packet_write_record",
        "create_dry_run_assignment_packet_record",
        "create_dry_run_assignment_audit_record",
        "create_dry_run_assignment_readiness_summary",
        "create_sandbox_worker_dry_run_result_candidate_bridge",
        "create_sandbox_worker_dry_run_assignment_candidate_bundle",
    ]:
        ensure(name in module and callable(module[name]), f"missing v5.7 module export: {name}")
...
def ensure_cli_flags() -> None:
    ...
    for flag in [
        "--sandbox-worker-dry-run-assignment-candidate-schema",
        "--sandbox-worker-dry-run-assignment-candidate",
        "--write-sandbox-worker-dry-run-assignment-candidate",
        "--v5-dry-run-sandbox-worker-label",
        "--v5-dry-run-handoff-packet-reference-label",
        "--v5-dry-run-acknowledgement-packet-reference-label",
        "--v5-dry-run-acceptance-review-packet-reference-label",
        "--v5-dry-run-ready-state-packet-reference-label",
        "--v5-dry-run-task-label",
        "--v5-dry-run-assignment-packet-name",
        "--v5-dry-run-assignment-confirm-token",
        "--v5-dry-run-assignment-human-operator",
    ]:
        ensure(flag in stdout, f"missing CLI flag: {flag}")


def ensure_schema_and_gates() -> None:
    schema = run_json(RUNTIME, ["--sandbox-worker-dry-run-assignment-candidate-schema"])
    ensure(schema["schema_version"] == "5.7.0", "schema version mismatch")
    ensure(schema["status"] == "SANDBOX_WORKER_DRY_RUN_ASSIGNMENT_CANDIDATE_LOCAL_PACKET_ONLY", "schema status mismatch")
    ensure(schema["dry_run_assignment_type"] == "sandbox_worker_dry_run_assignment_candidate", "schema type mismatch")
    for key in [
    ...
    for key in [
        "local_dry_run_assignment_packet_written",
        "sandbox_worker_dry_run_assignment_created",
    ...
    no_token = run_json(RUNTIME, [
        ...
        "--sandbox-worker-dry-run-assignment-candidate",
        "--v5-dry-run-sandbox-worker-label", SANDBOX_WORKER_LABEL,
        "--v5-dry-run-handoff-packet-reference-label", V5_3_REFERENCE_LABEL,
        "--v5-dry-run-acknowledgement-packet-reference-label", V5_4_REFERENCE_LABEL,
        "--v5-dry-run-acceptance-review-packet-reference-label", V5_5_REFERENCE_LABEL,
        "--v5-dry-run-ready-state-packet-reference-label", V5_6_REFERENCE_LABEL,
        "--v5-dry-run-task-label", DRY_RUN_TASK_LABEL,
        "--v5-dry-run-assignment-human-operator", HUMAN_OPERATOR,
        "--json",
    ])
    bundle = no_token.get("sandbox_worker_dry_run_assignment_candidate", {})
    ensure(bundle.get("approval_gate", {})["status"] == "BLOCKED_PENDING_V5_7_SANDBOX_WORKER_DRY_RUN_ASSIGNMENT_CANDIDATE_APPROVAL", "no-token gate should block")
    ensure(bundle.get("dry_run_assignment_packet_record", {}).get("write_record", {}).get("write_status") == "BLOCKED", "no-token write should block")
    ensure(bundle["local_dry_run_assignment_packet_written"] is False, "no-token bundle should not write")
    ensure(bundle["sandbox_worker_dry_run_assignment_created"] is False, "no-token bundle should not perform dry_run_assignment_candidate")
    ...
    bad_token = run_json(RUNTIME, [
        ...
        "--v5-dry-run-ready-state-packet-reference-label", V5_6_REFERENCE_LABEL,
        "--v5-dry-run-task-label", DRY_RUN_TASK_LABEL,
        "--v5-dry-run-assignment-confirm-token", "BAD_TOKEN",
        "--v5-dry-run-assignment-human-operator", HUMAN_OPERATOR,
        "--json",
    ])
    bundle = bad_token.get("sandbox_worker_dry_run_assignment_candidate", {})
    ensure(bundle.get("approval_gate", {})["status"] == "BLOCKED_PENDING_V5_7_SANDBOX_WORKER_DRY_RUN_ASSIGNMENT_CANDIDATE_APPROVAL", "bad-token gate should block")
    ensure(bundle.get("dry_run_assignment_packet_record", {}).get("write_record", {}).get("write_status") == "BLOCKED", "bad-token write should block")
    ...
    valid_no_write = run_json(RUNTIME, [
        ...
        "--v5-dry-run-ready-state-packet-reference-label", V5_6_REFERENCE_LABEL,
        "--v5-dry-run-task-label", DRY_RUN_TASK_LABEL,
        "--v5-dry-run-assignment-confirm-token", EXPECTED_TOKEN,
        "--v5-dry-run-assignment-human-operator", HUMAN_OPERATOR,
        "--json",
    ])
    bundle = valid_no_write.get("sandbox_worker_dry_run_assignment_candidate", {})
    gate = bundle.get("approval_gate", {})
    ensure(gate["status"] == "BLOCKED_PENDING_V5_7_SANDBOX_WORKER_DRY_RUN_ASSIGNMENT_CANDIDATE_APPROVAL", "valid token should approve one packet")
    ensure(gate["local_dry_run_assignment_records_authorized"] is True, "valid token should authorize local records")
    ensure(gate["local_dry_run_assignment_packet_write_authorized"] is False, "preview request absent should block write")
    ensure(bundle["v5_3_handoff_packet_reference_contract"]["status"] == "PASS", "handoff reference contract should pass")
    ensure(bundle["sandbox_worker_dry_run_assignment_reference_contract"]["status"] == "PASS", "worker contract should pass")
    ensure(bundle["dry_run_assignment_scope_contract"]["status"] == "PASS", "scope should pass")
    ensure(bundle["non_execution_dry_run_assignment_boundary"]["status"] == "PASS", "boundary should pass")
    ensure(bundle["dry_run_assignment_plan_record"]["status"] == "LOCAL_SANDBOX_WORKER_DRY_RUN_ASSIGNMENT_CANDIDATE_PLAN_CREATED", "plan should be created")
    ensure(bundle["dry_run_assignment_packet_record"]["status"] == "BLOCKED", "packet record should block without write request")
    ensure(bundle["dry_run_assignment_audit_record"]["status"] == "PASS", "audit should pass")
    ensure(bundle["dry_run_assignment_readiness_summary"]["status"] == "READY_FOR_SANDBOX_WORKER_DRY_RUN_RESULT_CANDIDATE_REVIEW_ONLY", "summary should be ready")
    ensure(bundle["sandbox_worker_dry_run_result_candidate_bridge"]["bridge_status"] == "READY", "bridge should be ready")
    ensure(bundle["dry_run_assignment_packet_record"]["write_record"]["write_status"] == "BLOCKED", "no-write path should block write")
    ensure(bundle["local_dry_run_assignment_packet_written"] is False, "no-write bundle should not write")
    ensure(bundle["sandbox_worker_dry_run_assignment_created"] is False, "no-write bundle should not perform dry_run_assignment_candidate")

    with tempfile.TemporaryDirectory(prefix="station_chief_v5_7_", dir="/tmp") as packet_dir_name:
        packet_dir = Path(packet_dir_name)
        write_result = run_json(RUNTIME, [
            "--command", "check please",
            "--write-sandbox-worker-dry-run-assignment-candidate", str(packet_dir),
            "--v5-dry-run-sandbox-worker-label", SANDBOX_WORKER_LABEL,
            "--v5-dry-run-handoff-packet-reference-label", V5_3_REFERENCE_LABEL,
            "--v5-dry-run-acknowledgement-packet-reference-label", V5_4_REFERENCE_LABEL,
            "--v5-dry-run-acceptance-review-packet-reference-label", V5_5_REFERENCE_LABEL,
            "--v5-dry-run-ready-state-packet-reference-label", V5_6_REFERENCE_LABEL,
            "--v5-dry-run-task-label", DRY_RUN_TASK_LABEL,
            "--v5-dry-run-assignment-packet-name", DEFAULT_PACKET_NAME,
            "--v5-dry-run-assignment-confirm-token", EXPECTED_TOKEN,
            "--v5-dry-run-assignment-human-operator", HUMAN_OPERATOR,
            "--json",
        ])
        bundle = write_result.get("sandbox_worker_dry_run_assignment_candidate", {})
        write_record = bundle.get("dry_run_assignment_packet_record", {}).get("write_record", {})
        
        ensure(write_record["write_status"] == "SANDBOX_WORKER_DRY_RUN_ASSIGNMENT_CANDIDATE_WRITTEN", "write path should write one packet")
        ensure(write_record["local_dry_run_assignment_packet_written"] is True, "write record should mark local write")
        ensure(write_record["sandbox_worker_dry_run_assignment_created"] is True, "write record should mark dry_run_assignment_candidate")
        ensure(write_record["files_written_count"] == 1, "write record should report exactly one file")
        ensure(bundle["local_dry_run_assignment_packet_written"] is True, "bundle should report local write")
        ensure(bundle["sandbox_worker_dry_run_assignment_created"] is True, "bundle should report dry_run_assignment_candidate")
        ensure(bundle["dry_run_assignment_audit_record"]["status"] == "PASS", "audit should pass in write path")
        ensure(bundle["dry_run_assignment_readiness_summary"]["status"] == "READY_FOR_SANDBOX_WORKER_DRY_RUN_RESULT_CANDIDATE_REVIEW_ONLY", "write summary should be ready")
        
        written_files = list(packet_dir.iterdir())
        ensure(len(written_files) == 1, f"expected exactly one dry_run_assignment_candidate, found {len(written_files)}")
        ...
        ensure(packet_payload["runtime_version"] == "5.7.0", "packet payload runtime version mismatch")
        ensure(packet_payload["dry_run_assignment_type"] == "sandbox_worker_dry_run_assignment_candidate", "packet payload type mismatch")
        ensure(packet_payload["dry_run_assignment_mode"] == "deterministic_local_dry_run_assignment_candidate_only", "packet payload mode mismatch")
        ensure(packet_payload["dry_run_assignment_message"] == "Station Chief sandbox worker dry-run assignment candidate wrote this deterministic local assignment packet. No worker was started and no task was executed.", "packet payload message mismatch")
        ensure(packet_payload["local_dry_run_assignment_packet_written"] is True, "packet payload write flag mismatch")
        ensure(packet_payload["sandbox_worker_dry_run_assignment_created"] is True, "packet payload dry_run_assignment_candidate flag mismatch")
        for key in [
            "worker_process_started",
            "agent_started",
            "real_queue_created",
...
def ensure_forbidden_patterns() -> None:
    code = V5_7_MODULE.read_text(encoding="utf-8")
    for pattern in FORBIDDEN_REGEXES:
        ensure(not re.search(pattern, code), f"forbidden pattern found in v5.7 module: {pattern}")
...
def ensure_changed_paths() -> None:
    ...
    unexpected = [p for p in changed_paths if p not in ALLOWED_CHANGED_PATHS and "v14" not in p and "v15" not in p and "v16" not in p and "v17" not in p and "v18" not in p and "v19" not in p and "v20" not in p and "v21" not in p and "v22" not in p and "v13" not in p and "validate_station_chief" not in p]
...
def ensure_runtime_wrapper_integration() -> None:
    ...
    res = runtime["attach_sandbox_worker_dry_run_assignment_candidate"](
        res,
    ...
    ensure("sandbox_worker_dry_run_assignment_candidate_bundle" in res, "attach should include bundle")
    ensure("dry_run_assignment_packet_record" in res, "attach should include packet record")
    ensure("sandbox_worker_dry_run_assignment_candidate" in res, "attach should include compatibility object")
    ensure(res["local_dry_run_assignment_packet_written"] is False, "attach no-write should not mark written")
    
    # 2. Test write
    with tempfile.TemporaryDirectory(prefix="station_chief_repair_v5_7_") as tmpdir:
        res2 = runtime["run_station_chief"]("check please")
        res2 = runtime["write_sandbox_worker_dry_run_assignment_candidate"](
            res2,
            output_dir=tmpdir,
            sandbox_worker_label=SANDBOX_WORKER_LABEL,
            v5_3_handoff_packet_reference_label=V5_3_REFERENCE_LABEL,
        ...
        ensure(res2["local_dry_run_assignment_packet_written"] is True, "write path should mark written")
        ensure(res2["sandbox_worker_dry_run_assignment_created"] is True, "assignment should be created")
        ensure(res2["dry_run_task_assignment_recorded"] is True, "assignment candidate should be recorded")
        ensure(res2["dry_run_result_created"] is False, "dry run result should not be created")
        ensure(res2["dry_run_task_executed"] is False, "dry run task should not be executed")
        ensure("sandbox_worker_dry_run_assignment_candidate_write_summary" in res2, "write path should include write summary")
        ensure("dry_run_assignment_packet_write_record" in res2, "write path should include packet write record")
        
        write_record = res2["dry_run_assignment_packet_write_record"]
        ensure(write_record["write_status"] == "SANDBOX_WORKER_DRY_RUN_ASSIGNMENT_CANDIDATE_WRITTEN", "write status mismatch")
        ...
        with open(packet_path) as pf:
            ...
            ensure(packet_data["runtime_version"] == "5.7.0", "payload version mismatch")
            ensure(packet_data["dry_run_assignment_type"] == "sandbox_worker_dry_run_assignment_candidate", "payload type mismatch")
            ensure(packet_data["local_dry_run_assignment_packet_written"] is True, "payload write flag mismatch")
            ensure(packet_data["sandbox_worker_dry_run_assignment_created"] is True, "payload created flag mismatch")
            ensure(packet_data["dry_run_task_assignment_recorded"] is True, "payload recorded flag mismatch")
            for key in ["dry_run_result_created", "dry_run_task_executed", "worker_process_started", "agent_started", "task_executed"]:
                ensure(packet_data.get(key) is False, f"payload dangerous flag {key} must be false")
        
        # Verify dangerous booleans in result
        for key in ["dry_run_result_created", "worker_process_started", "agent_started"]:
...
def ensure_smoke_tests() -> None:
    if os.environ.get("STATION_CHIEF_SKIP_RECURSIVE_VALIDATION") == "1":
        return
    for validator in [V5_6_VALIDATOR, V5_4_VALIDATOR, V5_3_VALIDATOR, V5_2_VALIDATOR, V5_1_VALIDATOR, V5_0_VALIDATOR]:
...
def main() -> None:
    ...
    ensure_no_v62_files()
    print("STATION_CHIEF_RUNTIME_V5_7_VALIDATION_PASS")
