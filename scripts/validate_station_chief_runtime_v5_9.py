...
def ensure_doctrine() -> None:
    ...
    ensure("Station Chief Runtime upgraded to v5.9.0" in readme_skeleton_content, "Missing upgrade doctrine in README/Skeleton")
...
def ensure_protected_paths() -> None:
    ...
    forbidden_indicators = ["devinization", "ownership", "credential", "secret", "env", "production", "deployment", "v6_0", "v6.0", "mvp", "lock"]
...
def ensure_no_v6_files() -> None:
    print("Checking for unexpected v6.0/MVP files...")
    v6_files = list(REPO_ROOT.rglob("*v6_0*")) + list(REPO_ROOT.rglob("*v6.0*")) + list(REPO_ROOT.rglob("*mvp*lock*"))
    # Filter out text matches inside files if rglob returns them (it shouldn't, but being safe)
    actual_files = [f for f in v6_files if f.is_file() or f.is_dir()]
    # Exclude v5.9 report which might mention v6.0 in its text (rglob on name should be fine though)
    ensure(len(actual_files) == 0, f"Unexpected v6.0/MVP lock files found: {actual_files}")
