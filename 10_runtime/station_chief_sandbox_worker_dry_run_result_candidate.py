"""
Station Chief Runtime v5.8.0 Sandbox Worker Dry-Run Result Candidate Module
"""
...
SANDBOX_WORKER_DRY_RUN_RESULT_CANDIDATE_MODULE_VERSION = "5.8.0"
SANDBOX_WORKER_DRY_RUN_RESULT_CANDIDATE_STATUS = "SANDBOX_WORKER_DRY_RUN_RESULT_CANDIDATE_LOCAL_PACKET_ONLY"
SANDBOX_WORKER_DRY_RUN_RESULT_CANDIDATE_PHASE = "Sandbox Worker Dry-Run Result Candidate"
SANDBOX_WORKER_DRY_RUN_RESULT_CANDIDATE_APPROVAL_TOKEN = "YES_I_APPROVE_SANDBOX_WORKER_DRY_RUN_RESULT_CANDIDATE"
...
DEFAULT_SANDBOX_DRY_RUN_RESULT_PACKET_NAME = "sandbox_worker_dry_run_result_candidate_packet.json"
...
def safe_dry_run_result_packet_name(packet_name: str | None) -> str:
    if not packet_name:
        return DEFAULT_SANDBOX_DRY_RUN_RESULT_PACKET_NAME
...
def generate_sandbox_worker_dry_run_result_candidate_id(
    command: str,
    ...
    norm_assignment = normalize_label(v5_7_dry_run_assignment_packet_reference_label, DEFAULT_V5_7_DRY_RUN_ASSIGNMENT_PACKET_REFERENCE_LABEL)
    norm_task = normalize_label(synthetic_dry_run_task_label, DEFAULT_SYNTHETIC_DRY_RUN_TASK_LABEL)
    norm_result = normalize_label(synthetic_dry_run_result_label, DEFAULT_SYNTHETIC_DRY_RUN_RESULT_LABEL)
    h = sha256_digest({
        "command": command,
        "worker": norm_worker,
    ...
    return f"sandbox-worker-dry-run-result-candidate-v5-8-{norm_worker}-{norm_handoff}-{norm_ack}-{norm_acceptance}-{norm_ready}-{norm_assignment}-{norm_task}-{norm_result}-{h}"


def create_sandbox_worker_dry_run_result_candidate_schema() -> dict:
    return {
        "schema_version": "5.8.0",
        "status": "SANDBOX_WORKER_DRY_RUN_RESULT_CANDIDATE_LOCAL_PACKET_ONLY",
        "dry_run_result_type": "sandbox_worker_dry_run_result_candidate",
        "required_sections": [
        ...
        "required_token": SANDBOX_WORKER_DRY_RUN_RESULT_CANDIDATE_APPROVAL_TOKEN,
        "baseline_preserved": True,
        "local_dry_run_result_packet_written": False,
        "sandbox_worker_dry_run_result_candidate_created": False,
        "dry_run_result_candidate_recorded": False,
        "dry_run_task_executed": False,
        "real_worker_result_created": False,
        "replay_audit_performed": False,
...
def create_sandbox_worker_dry_run_result_approval_gate(
    sandbox_worker_label: str,
    v5_3_handoff_packet_reference_label: str,
    v5_4_acknowledgement_packet_reference_label: str,
    v5_5_acceptance_review_packet_reference_label: str,
    ...
    token_valid = (confirmation_token == SANDBOX_WORKER_DRY_RUN_RESULT_CANDIDATE_APPROVAL_TOKEN)
    ...
    has_assignment = bool(v5_7_dry_run_assignment_packet_reference_label)
    has_task = bool(synthetic_dry_run_task_label)
    has_result = bool(synthetic_dry_run_result_label)
    has_outdir = bool(output_directory)

    records_auth = bool(token_valid and has_operator and has_worker and has_handoff and has_ack and has_acceptance and has_ready and has_assignment and has_task and has_result)
    write_auth = bool(records_auth and has_outdir and dry_run_result_requested)

    status = "APPROVED_FOR_ONE_LOCAL_SANDBOX_WORKER_DRY_RUN_RESULT_CANDIDATE" if write_auth else "BLOCKED_PENDING_V5_8_SANDBOX_WORKER_DRY_RUN_RESULT_CANDIDATE_APPROVAL"

    return {
        "status": status,
        "local_dry_run_result_records_authorized": records_auth,
        "local_dry_run_result_packet_write_authorized": write_auth,
...
def create_v5_3_handoff_packet_reference_contract(approval_gate: dict) -> dict:
    if not approval_gate.get("local_dry_run_result_records_authorized"):
...
def create_v5_6_ready_state_packet_reference_contract(approval_gate: dict) -> dict:
    ...
    return {
        ...
        "reference_does_not_execute_tasks": True,
        "reference_does_not_create_real_result": True
    }
...
def create_v5_7_dry_run_assignment_packet_reference_contract(approval_gate: dict) -> dict:
    ...
    return {
        "status": "PASS",
        "references_exactly_one_v5_7_dry_run_assignment_packet_label": True,
        "reference_is_metadata_only": True,
        "reference_is_not_read_from_disk": True,
        "reference_is_not_mutated": True,
        "reference_is_not_executed": True,
        ...
        "reference_is_not_routed": True,
        "reference_does_not_start_worker": True,
        "reference_does_not_execute_dry_run_task": True,
        "reference_does_not_create_real_result": True,
        "reference_does_not_perform_replay_audit": True
    }
...
def create_synthetic_dry_run_task_reference_contract(approval_gate: dict) -> dict:
    ...
    return {
        ...
        "task_label_cannot_produce_a_real_result_in_v5_8": True
    }
...
def create_synthetic_dry_run_result_reference_contract(approval_gate: dict) -> dict:
    ...
    return {
        "status": "PASS",
        "references_exactly_one_synthetic_dry_run_result_label": True,
        "result_label_is_metadata_only": True,
        "result_label_is_deterministic": True,
        "result_label_is_not_produced_by_task_execution": True,
        "result_label_is_not_a_real_worker_output": True,
        "result_label_is_not_read_from_disk": True,
        "result_label_is_not_mutated": True,
        "result_label_is_not_executed": True,
        "result_label_is_not_sent_externally": True,
        "result_label_cannot_call_tools": True,
        "result_label_cannot_access_apis": True,
        "result_label_cannot_access_network": True,
        "result_label_cannot_access_credentials_secrets_environment": True,
        "result_label_is_for_later_v5_9_replay_audit_review_only": True
    }


def create_sandbox_worker_dry_run_result_reference_contract(approval_gate: dict) -> dict:
    if not approval_gate.get("local_dry_run_result_records_authorized"):
        return {"status": "BLOCKED"}
    return {
        "status": "PASS",
        ...
        "worker_dry_run_result_candidate_is_local_metadata_only": True,
        "worker_executes_no_task": True,
        "worker_produces_no_real_result": True,
        ...
        "worker_cannot_perform_replay_audit_in_v5_8": True
    }


def create_dry_run_result_scope_contract(
    approval_gate: dict,
    v5_3_contract: dict,
    v5_4_contract: dict,
    v5_5_contract: dict,
    ...
    records_auth = approval_gate.get("local_dry_run_result_records_authorized", False)
    all_contracts_pass = (
        ...
        v5_7_contract.get("status") == "PASS" and
        task_contract.get("status") == "PASS" and
        result_contract.get("status") == "PASS" and
    ...
    return {
        ...
        "exactly_one_dry_run_result_candidate_packet": True,
        "explicit_output_directory_required": True,
        "packet_json_only": True,
        "dry_run_result_metadata_only": True,
        "synthetic_result_label_only": True,
        "no_dry_run_task_execution": True,
        "no_real_worker_result": True,
        "no_replay_audit": True,
...
def create_non_execution_dry_run_result_boundary(
    approval_gate: dict,
    dry_run_result_scope_contract: dict
) -> dict:
    if dry_run_result_scope_contract.get("status") != "PASS":
        return {"status": "BLOCKED"}
    
    write_auth = approval_gate.get("local_dry_run_result_packet_write_authorized", False)

    return {
        "status": "PASS",
        "sandbox_worker_dry_run_result_candidate_is_local_packet_only": True,
        "dry_run_result_candidate_is_not_executed": True,
        "dry_run_task_is_not_executed": True,
        "real_worker_result_is_not_created": True,
        "replay_audit_is_not_performed": True,
        "synthetic_task_label_is_not_enqueued": True,
        "synthetic_result_label_is_not_externally_transmitted": True,
        "worker_is_not_started": True,
        ...
        "sandbox_worker_dry_run_result_candidate_created": write_auth,
        "dry_run_result_candidate_recorded": write_auth,
        "worker_process_started": False,
        "agent_started": False,
        "real_queue_created": False,
        "queue_write_performed": False,
...
def create_dry_run_result_permission_denial_record(
    sandbox_worker_label: str,
    v5_3_handoff_packet_reference_label: str,
    v5_4_acknowledgement_packet_reference_label: str,
    v5_5_acceptance_review_packet_reference_label: str,
    ...
    return {
        "denied_permissions": [
            ...
            "real dry-run result generation",
            "real worker result creation",
            "replay/audit execution",
            ...
            "mutation of the referenced v5.7 dry-run assignment packet",
            "execution of the referenced v5.7 dry-run assignment packet",
            "execution of the synthetic dry-run task label",
            "external transmission of the synthetic dry-run result label"
        ]
    }


def create_dry_run_result_plan_record(
    approval_gate: dict,
    v5_3_contract: dict,
    v5_4_contract: dict,
    v5_5_contract: dict,
    ...
    if (approval_gate.get("local_dry_run_result_records_authorized") and
        v5_3_contract.get("status") == "PASS" and
        v5_4_contract.get("status") == "PASS" and
        v5_5_contract.get("status") == "PASS" and
        v5_6_contract.get("status") == "PASS" and
        ...
        status = "LOCAL_SANDBOX_WORKER_DRY_RUN_RESULT_CANDIDATE_PLAN_CREATED"
    else:
        status = "BLOCKED"
    ...
    return {
        "status": status,
        "dry_run_result_candidate_id": candidate_id,
        ...
        "v5_7_dry_run_assignment_packet_reference_label_normalized": normalize_label(v5_7_dry_run_assignment_packet_reference_label, DEFAULT_V5_7_DRY_RUN_ASSIGNMENT_PACKET_REFERENCE_LABEL),
        ...
        "synthetic_dry_run_result_label_normalized": normalize_label(synthetic_dry_run_result_label, DEFAULT_SYNTHETIC_DRY_RUN_RESULT_LABEL),
        "human_operator": human_operator,
        "dry_run_result_mode": "deterministic_local_dry_run_result_candidate_only",
        "packet_runtime_state": "written" if write_auth else "not_written",
        "worker_runtime_state": "not_started",
        "dry_run_result_state": "recorded_synthetic_metadata_only" if write_auth else "not_created",
        "dry_run_task_execution_state": "not_executed",
        "replay_audit_state": "not_performed",
...
def build_dry_run_result_packet_payload(
    sandbox_worker_label: str,
    v5_3_handoff_packet_reference_label: str,
    v5_4_acknowledgement_packet_reference_label: str,
    v5_5_acceptance_review_packet_reference_label: str,
    ...
    payload = {
        ...
        "dry_run_result_candidate_id": candidate_id,
        "dry_run_result_message": "Station Chief sandbox worker dry-run result candidate wrote this deterministic local result packet. No worker was started and no task was executed.",
        "dry_run_result_value": "SYNTHETIC_DRY_RUN_RESULT_RECORDED_FOR_REPLAY_AUDIT_REVIEW_ONLY",
        "dry_run_result_next_review": "READY_FOR_SANDBOX_WORKER_DRY_RUN_REPLAY_AUDIT_CANDIDATE_REVIEW_ONLY",
        "local_dry_run_result_packet_written": True,
        "sandbox_worker_dry_run_result_candidate_created": True,
        "dry_run_result_candidate_recorded": True,
        "dry_run_task_executed": False,
        "real_worker_result_created": False,
        "replay_audit_performed": False,
        "worker_process_started": False,
...
def write_sandbox_worker_dry_run_result_packet(
    ...
    try:
        ...
        safe_name = safe_dry_run_result_packet_name(packet_name)
        packet_path = out_dir / safe_name
        
        # Ensure it's inside output_directory
        if not str(packet_path.resolve()).startswith(str(out_dir)):
            return create_blocked_dry_run_result_packet_write_record("path traversal prevented")
        
        packet_path.write_text(canonical_json(payload))
        
        return {
            "sandbox_worker_dry_run_result_write_record_version": "5.8.0",
            "write_status": "SANDBOX_WORKER_DRY_RUN_RESULT_CANDIDATE_WRITTEN",
            "local_dry_run_result_packet_written": True,
            "sandbox_worker_dry_run_result_candidate_created": True,
            "dry_run_result_candidate_recorded": True,
            "dry_run_task_executed": False,
        ...
        return create_blocked_dry_run_result_packet_write_record(f"write failed: {e}")


def create_blocked_dry_run_result_packet_write_record(reason: str) -> dict:
    return {
...
def create_dry_run_result_packet_record(write_record: dict, payload_digest: str | None) -> dict:
    if write_record.get("write_status") == "SANDBOX_WORKER_DRY_RUN_RESULT_CANDIDATE_WRITTEN":
        status = "SANDBOX_WORKER_DRY_RUN_RESULT_CANDIDATE_WRITTEN"
    else:
        status = "BLOCKED"
    ...
    return {
        ...
        "local_dry_run_result_packet_written": write_record.get("local_dry_run_result_packet_written", False),
        "sandbox_worker_dry_run_result_candidate_created": write_record.get("sandbox_worker_dry_run_result_candidate_created", False),
        "dry_run_result_candidate_recorded": write_record.get("dry_run_result_candidate_recorded", False),
        "dry_run_task_executed": False,
        "real_worker_result_created": False,
        "replay_audit_performed": False,
        "worker_process_started": False,
...
def create_dry_run_result_audit_record(
    approval_gate: dict,
    v5_3_contract: dict,
    v5_4_contract: dict,
    v5_5_contract: dict,
    ...
    for rec in [plan_record, packet_record]:
        ...
        if rec.get("arbitrary_task_execution_performed") or rec.get("user_task_execution_performed"):
            pass_audit = False
        if rec.get("dry_run_task_executed") or rec.get("real_worker_result_created") or rec.get("replay_audit_performed"):
            pass_audit = False
    ...
    return {
        ...
        "section_digests": {
            ...
            "v5_7_dry_run_assignment_packet_reference_contract": sha256_digest(v5_7_contract),
            "synthetic_dry_run_task_reference_contract": sha256_digest(task_contract),
            "synthetic_dry_run_result_reference_contract": sha256_digest(result_contract),
            "sandbox_worker_dry_run_result_reference_contract": sha256_digest(worker_contract),
            "dry_run_result_scope_contract": sha256_digest(scope_contract),
            "non_execution_dry_run_result_boundary": sha256_digest(boundary),
            "dry_run_result_permission_denial_record": sha256_digest(denial_record),
            "dry_run_result_plan_record": sha256_digest(plan_record),
            "dry_run_result_packet_record": sha256_digest(packet_record)
        },
        "sandbox_worker_dry_run_result_candidate_created": write_auth,
        "dry_run_result_candidate_recorded": write_auth,
        "dry_run_task_executed": False,
...
def create_dry_run_result_readiness_summary(audit_record: dict) -> dict:
    if audit_record.get("status") == "PASS":
        status = "READY_FOR_SANDBOX_WORKER_DRY_RUN_REPLAY_AUDIT_CANDIDATE_REVIEW_ONLY"
    else:
        status = "BLOCKED"
        
    return {
        "status": status,
        "next_layer": "Sandbox Worker Dry-Run Replay / Audit Candidate",
        "v5_9_not_built": True,
        "one_deterministic_sandbox_worker_dry_run_result_candidate_packet_permitted_only_under_v5_8_token": True,
        "no_dry_run_task_execution": True,
        "no_real_worker_result": True,
        "no_replay_audit": True,
        "no_worker_start": True,
...
def create_sandbox_worker_dry_run_replay_audit_candidate_bridge(summary: dict) -> dict:
    return {
        "bridge_status": "READY" if summary.get("status") == "READY_FOR_SANDBOX_WORKER_DRY_RUN_REPLAY_AUDIT_CANDIDATE_REVIEW_ONLY" else "BLOCKED",
        "bridge_to_v5_9_review_only": True,
        "no_sandbox_worker_dry_run_replay_audit_in_v5_8": True,
        "no_dry_run_task_execution_in_v5_8": True,
        "no_worker_start_in_v5_8": True,
        "no_agent_start_in_v5_8": True,
        "no_queue_creation_in_v5_8": True,
        "no_task_enqueue_in_v5_8": True,
        "no_arbitrary_task_execution_in_v5_8": True,
        "no_user_task_execution_in_v5_8": True,
        "no_worker_routing_in_v5_8": True,
        "worker_process_started": False,
        "agent_started": False,
        "real_queue_created": False,
        "queue_write_performed": False,
...
def create_sandbox_worker_dry_run_result_candidate_bundle(
    result: dict | None,
    ...
    assignment_label = v5_7_dry_run_assignment_packet_reference_label or ""
    task_label = synthetic_dry_run_task_label or ""
    result_label = synthetic_dry_run_result_label or ""
    
    approval_gate = create_sandbox_worker_dry_run_result_approval_gate(
        worker_label, handoff_label, ack_label, acceptance_label, ready_label, assignment_label, task_label, result_label, output_directory,
        confirmation_token, human_operator, dry_run_result_requested
    )
    ...
    v5_7_contract = create_v5_7_dry_run_assignment_packet_reference_contract(approval_gate)
    task_contract = create_synthetic_dry_run_task_reference_contract(approval_gate)
    result_contract = create_synthetic_dry_run_result_reference_contract(approval_gate)
    worker_contract = create_sandbox_worker_dry_run_result_reference_contract(approval_gate)
    scope = create_dry_run_result_scope_contract(approval_gate, v5_3_contract, v5_4_contract, v5_5_contract, v5_6_contract, v5_7_contract, task_contract, result_contract, worker_contract)
    boundary = create_non_execution_dry_run_result_boundary(approval_gate, scope)
    denial = create_dry_run_result_permission_denial_record(worker_label, handoff_label, ack_label, acceptance_label, ready_label, assignment_label, task_label, result_label)
    
    candidate_id = generate_sandbox_worker_dry_run_result_candidate_id(
        command_str, worker_label, handoff_label, ack_label, acceptance_label, ready_label, assignment_label, task_label, result_label
    )
    
    plan = create_dry_run_result_plan_record(
        approval_gate, v5_3_contract, v5_4_contract, v5_5_contract, v5_6_contract, v5_7_contract, task_contract, result_contract, worker_contract, scope, boundary,
        candidate_id, worker_label, handoff_label, ack_label, acceptance_label, ready_label, assignment_label, task_label, result_label, human_operator or ""
    )

    write_record = None
    payload_digest = None
    
    if write_dry_run_result_packet:
        if plan.get("status") == "LOCAL_SANDBOX_WORKER_DRY_RUN_RESULT_CANDIDATE_PLAN_CREATED" and approval_gate.get("local_dry_run_result_packet_write_authorized"):
            payload = build_dry_run_result_packet_payload(
                worker_label, handoff_label, ack_label, acceptance_label, ready_label, assignment_label, task_label, result_label, human_operator or "", candidate_id, True
            )
            payload_digest = payload["payload_digest"]
            write_record = write_sandbox_worker_dry_run_result_packet(
                output_directory or "", dry_run_result_packet_name or "", payload
            )
        else:
            write_record = create_blocked_dry_run_result_packet_write_record("requirements not met")
    else:
        write_record = create_blocked_dry_run_result_packet_write_record("sandbox worker dry-run result candidate packet write not requested")

    packet_record = create_dry_run_result_packet_record(write_record, payload_digest)
    
    audit = create_dry_run_result_audit_record(
        approval_gate, v5_3_contract, v5_4_contract, v5_5_contract, v5_6_contract, v5_7_contract, task_contract, result_contract, worker_contract, scope, boundary, denial, plan, packet_record
    )
    summary = create_dry_run_result_readiness_summary(audit)
    bridge = create_sandbox_worker_dry_run_replay_audit_candidate_bridge(summary)

    bundle = {
        "schema": create_sandbox_worker_dry_run_result_candidate_schema(),
        ...
        "v5_7_dry_run_assignment_packet_reference_contract": v5_7_contract,
        "synthetic_dry_run_task_reference_contract": task_contract,
        "synthetic_dry_run_result_reference_contract": result_contract,
        "sandbox_worker_dry_run_result_reference_contract": worker_contract,
        "dry_run_result_scope_contract": scope,
        "non_execution_dry_run_result_boundary": boundary,
        "dry_run_result_permission_denial_record": denial,
        "dry_run_result_plan_record": plan,
        "dry_run_result_packet_record": packet_record,
        "dry_run_result_audit_record": audit,
        "dry_run_result_readiness_summary": summary,
        "sandbox_worker_dry_run_replay_audit_candidate_bridge": bridge,
        "local_dry_run_result_packet_written": packet_record.get("local_dry_run_result_packet_written", False),
        "sandbox_worker_dry_run_result_candidate_created": packet_record.get("sandbox_worker_dry_run_result_candidate_created", False),
        "dry_run_result_candidate_recorded": packet_record.get("dry_run_result_candidate_recorded", False),
        "dry_run_task_executed": False,
        "real_worker_result_created": False,
        "replay_audit_performed": False,
        "worker_process_started": False,
