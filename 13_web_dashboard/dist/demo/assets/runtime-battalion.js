(function () {
  const ENDPOINTS = {
    list: "/.netlify/functions/list-runtime-battalion",
    activateBattalion: "/.netlify/functions/activate-runtime-battalion",
    deactivateBattalion: "/.netlify/functions/deactivate-runtime-battalion",
    activateLane: "/.netlify/functions/activate-agent-lane",
    deactivateLane: "/.netlify/functions/deactivate-agent-lane",
    heartbeat: "/.netlify/functions/battalion-heartbeat",
    readinessNote: "/.netlify/functions/create-battalion-readiness-note",
  };
  ...
  const state = {
    ...
    readinessNotes: [],
    auditEvents: [],
    selectedLaneKey: "intake_lane",
    selectedAgentId: "mvp55_battalion_agent_001",
  };
  ...
  const ids = {
    ...
    runtimeBattalionSizeState: "runtimeBattalionSizeState",
  ...
  function renderBackendPanel() {
    ...
    byId(ids.runtimeBattalionSizeState).textContent = state.backendStatus ? String(Number(state.backendStatus.runtime_battalion_size || 100)) : "100";
    byId(ids.liveRuntimeAgentsState).textContent = state.backendStatus ? `${Number(state.backendStatus.live_runtime_agents_enabled || 0)} of 100` : "0 of 100";
    byId(ids.maxBatchSizeState).textContent = state.backendStatus ? String(Number(state.backendStatus.max_activation_batch_size || 100)) : "100";
    byId(ids.activeLanesState).textContent = state.backendStatus ? `${Number(state.backendStatus.active_lanes_count || 0)} of 10` : "0 of 10";
    byId(ids.inactiveLanesState).textContent = state.backendStatus ? `${Number(state.backendStatus.inactive_lanes_count || 10)} of 10` : "10 of 10";
  ...
  function renderLaneRoster() {
    ...
    if (!state.lanes.length) {
      roster.innerHTML = '<div class="empty-state">No battalion lanes loaded yet.</div>';
      return;
    }

    roster.innerHTML = state.lanes
      .map((lane) => {
        ...
        const heartbeatCount = state.heartbeatEvents.filter((event) => event.lane_key === lane.lane_key).length;
        const readinessCount = state.readinessNotes.filter((note) => note.lane_key === lane.lane_key).length;
  ...
  function renderSelectedPanels() {
    ...
    const laneId = byId(ids.currentLaneId);
    const agentMeta = byId(ids.currentAgentMeta);
    const agentId = byId(ids.currentAgentId);
    ...
    if (laneId) laneId.textContent = lane ? lane.lane_key : "intake_lane";
    if (agentMeta) {
    ...
    if (agentId) agentId.textContent = agent ? agent.agent_id : "mvp55_battalion_agent_001";
    if (permissions) {
  ...
  async function loadState() {
    setMessage("Loading runtime battalion state...", "info");
    try {
  ...
  function buildBattalionAgentIds() {
  ...
  async function submitAction(url, payload, successMessage) {
    ...
    try {
      ...
      if (response.status === 503 || data.error === "Runtime battalion backend is not configured.") {
        setBackendUnavailable(backendUnavailableMessage);
        return;
      }
  ...
  function bindEvents() {
    ...
    byId("activateBattalionButton")?.addEventListener("click", () => {
      submitAction(ENDPOINTS.activateBattalion, {
        agent_ids: buildBattalionAgentIds(),
        actor_name: "operator",
        reason: "Controlled 100-agent battalion activation",
        batch_size: 100,
      }, "Activate full battalion");
    });
    byId("deactivateBattalionButton")?.addEventListener("click", () => {
      submitAction(ENDPOINTS.deactivateBattalion, {
        agent_ids: buildBattalionAgentIds(),
        actor_name: "operator",
        reason: "Controlled 100-agent battalion deactivation",
        batch_size: 100,
    ...
    byId("heartbeatButton")?.addEventListener("click", () => {
      submitAction(ENDPOINTS.heartbeat, readHeartbeatPayload(), "Send battalion heartbeat");
    });
